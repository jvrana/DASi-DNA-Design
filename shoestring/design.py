...
class Design(object):
    ...
    def add(self, primers_path, template_path, query_path):
        self.add_primers(primers_path)
        self.add_templates(template_path)
        self.add_queries(query_path)

    def add_primers(self, path: str):
        self.logger.info("Adding primers from '{}'".format(path))
        primers = make_linear(
            load_fasta_glob(path)
        )
        self.factory.add_records(primers, self.PRIMERS)

    def add_templates(self, path: str):
        self.logger.info("Adding templates from '{}'".format(path))
        templates = load_genbank_glob(path)
        self.factory.add_records(templates, self.TEMPLATES)

    def add_queries(self, path: str):
        self.logger.info("Adding queries from '{}'".format(path))
        queries = make_circular(
            load_genbank_glob(path)
        )
